...
class SecretaryOutbox extends Controller
{
    ...
    public function upload_mail(Request $request)
    {
        ...
        $fileModel =  OutboxMail::find($request->id);
        if ($request->file()) {
    ...
    public function exportPDF(Request $request)
    {
        ...
        $autograph = Autograph::where('id', $mail->autograph_id)
            ->first();
        $data = [
            'mail' => $mail,
            'ttd' => $autograph
        ...
        $pdf = PDF::loadView('siswa.secretary.export_outbox', ['mail' => $mail, 'ttd' => $autograph]);
